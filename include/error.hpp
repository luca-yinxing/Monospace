...
enum logtime { LOG_NOTIME = 0, LOG_TIME_ISO8601 = 1 };

inline void print_log(loglevel lvl, logtime time_fmt, const char *fmt, ...)
{
        ...
        if (time_fmt != LOG_NOTIME) {
                ...
                if (time_fmt == LOG_TIME_ISO8601)
                        strftime(buffer, STRING_TIME_LENGTH_MAX, "[%T]",
                                 timeinfo);
