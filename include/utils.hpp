 ...
 */
inline int str16len(const char16_t *src) noexcept
{
        if (src == nullptr) {
                return -1;
        }
 ...
 */
inline int str16ncpy(char16_t *dest, const char16_t *src, int len) noexcept
{
 ...
 * \return int how many char are copied
 */
inline int str16cpy(char16_t *dest, char16_t *src) noexcept
{
        if (dest == nullptr || src == nullptr)
                return -1;
...
inline char *new_string(int length = DEF_BUFFER_STR) noexcept
{
        if (length < 0)
                return nullptr;
...
inline char16_t *new_string16(int length = DEF_BUFFER_STR) noexcept
{
        if (length < 0)
                return nullptr;
...
inline void *new_array(int items, int szof) noexcept
{
        if (items <= 0 || szof <= 0)
                return nullptr;
...
inline void *realloc_array(void *ptr, int items, int szof) noexcept
{
        if (items <= 0 || szof <= 0)
                return nullptr;
...
inline void *new_array_zero(int items, int szof) noexcept
{
        if (items <= 0 || szof <= 0)
                return nullptr;
...
inline void *new_memory(int memsize) noexcept
{
        if (memsize <= 0)
                return nullptr;
...
inline void *new_memory_zero(int memsize) noexcept
{
        if (memsize <= 0)
                return nullptr;
...
inline void *realloc_memory(void *ptr, size_t memsize) noexcept
{
        if (memsize <= 0)
                return nullptr;
...
inline char *new_string_copy(const char *src) noexcept
{
        if (src == nullptr)
                return nullptr;
...
inline char16_t *new_string16_copy(const char16_t *src) noexcept
{
        if (src == nullptr)
                return nullptr;
...
inline void *new_memory_copy(const void *src, int length)
{
...
inline void *new_array_copy(const void *src, int items, int szof)
{
...
inline void *new_array_zero_copy(const void *src, int items, int szof) noexcept
{
        if (src == nullptr || items <= 0 || szof <= 0)
                return nullptr;
...
inline execode free_memory(void *mem) noexcept
{
        if (mem == nullptr)
                return EXE_ERROR;
 ...
 */
inline uint32_t to_ui32(uint8_t *mem, uint32_t start = 0) noexcept
{
 ...
 */
inline execode to_utf8(const char16_t *src, char *dest,
...
inline execode to_utf16(const char src[], char16_t dest[],
                        int maxlen = DEF_BUFFER_STR) noexcept
{
        if (src == nullptr || dest == nullptr)
                return EXE_INVALID_ARGS;
...
inline execode strconcat(char *buffer, const char *s1, const char *s2) noexcept
{
...
inline execode strconcat(char *buffer, const char *s1, const char *s2,
...
inline execode strconcat(char *buffer, const char **items,
                         int items_num) noexcept
{
...
inline execode check_unique_array_int(const int *data, size_t count) noexcept
{
        if (data == nullptr) {
                return EXE_INVALID_ARGS;
        }
...
inline execode check_unique_array_int(const int *first, size_t first_count,
                                      const int *second,
...
inline bool strok(const char *str) noexcept
{
...
inline execode os_mkdir(const std::string &path, uint mode = 0755)
{
...
inline execode strlower(char *str)
{
        if (str == NULL) {
                return EXE_INVALID_ARGS;
        }
...
inline execode strlower(const char *src, char *dest)
{
...
inline execode strlists(char *buffer, const char **items, size_t items_num,
                        size_t start = 0, const char *sep = ",") noexcept
{
