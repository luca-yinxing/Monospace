...
static int s_find_field(uint32_t klass_addr, uint32_t klass_parent,
                        monoclass_kind klass_kind, uint32_t klass_fieldcount,
                        const char *name)
{
...
static int s_field_list(uint32_t klass_addr, uint32_t klass_parent,
                        monoclass_kind klass_kind, uint32_t klass_fieldcount,
                        uint32_t *field_list, int maxlen, int start_index = 0)
{
        int pushed_item = 0;
        ...
        if (klass_parent != ADDR_ZERO) {
                if (get_class_fieldcount(klass_parent) > 0) {
                        ...
                        pushed_item +=
                                s_field_list(klass_parent, parent_parent,
                                             parent_kind, parent_fieldcount,
                                             field_list, maxlen - pushed_item,
        ...
        if (klass_kind == GINST) {
                ...
                if (get_class_fieldcount(ginst_addr) > 0) {
                        ...
                        pushed_item +=
                                s_field_list(ginst_addr, ginst_parent,
                                             ginst_kind, ginst_fieldcount,
                                             field_list, maxlen - pushed_item,
                                             start_index + pushed_item);
                }
        }
...
int get_field_list(uint32_t klass_addr, uint32_t *field_list,
                   int maxlen) noexcept
{
        ...
        return s_field_list(klass_addr, klass_parent, klass_kind,
                            klass_fieldcount, field_list, maxlen);
}

int get_field_list(const monoclass &klass, uint32_t *field_list,
                   int maxlen) noexcept
{
        return s_field_list(klass.address, klass.parent, klass.kind,
                            klass.fieldcount, field_list, maxlen);
}

int get_field_list(const monovalue &value, uint32_t *field_list,
                   int maxlen) noexcept
{
...
uint32_t find_field(uint32_t klass_addr, const char *name) noexcept
{
        ...
        return s_find_field(klass_addr, klass_parent, klass_kind,
                            klass_fieldcount, name);
}

uint32_t find_field(const monoclass &klass, const char *name) noexcept
{
        return s_find_field(klass.address, klass.parent, klass.kind,
                            klass.fieldcount, name);
}

static execode s_get_field_value(const monovalue &klass_v, uint32_t field_addr,
                                 monovalue &value) noexcept
{
        ...
        if (is_value_class_def(klass_v)) {
                ...
                uint32_t vtable_data_addr = get_class_data_addr(
                        klass_vtable_addr, klass_vtable_size);

                uint32_t offset_addr = get_vtable_field_offset(
                        vtable_data_addr, field_offset, is_value_type);
        ...
        uint32_t offset_addr = get_vtable_field_offset(
                klass_v.klass_data, field_offset, is_value_type);

        return get_value(offset_addr, field_type, value);
}

execode get_field_value(const monovalue &klass_v, const char *name,
                        monovalue &value) noexcept
{
...
execode get_field_value(const monovalue &klass_v, uint32_t field_addr,
                        monovalue &value) noexcept
{
        return s_get_field_value(klass_v, field_addr, value);
}

execode get_field_value_string16(const monovalue &klass_v, const char *name,
                                 char16_t *str, int length) noexcept
{
...
execode get_field_value_string(const monovalue &klass_v, const char *name,
                               char *str, int length) noexcept
{
        if (str == nullptr || length < 1) {
                return EXE_INVALID_ARGS;
        }
...
execode get_field_value_int(const monovalue &klass_v, const char *name,
                            int &dest) noexcept
{
        execode retval = EXE_OK;
        monovalue tmp;
...
execode get_field_value_ui64(const monovalue &klass_v, const char *name,
...
execode get_field_value_i64(const monovalue &klass_v, const char *name,
