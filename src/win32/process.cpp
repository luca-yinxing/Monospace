...
execode find_module(int pid, moduleinfo *m_info, const char *m_name)
{
        ...
        if (!EnumProcessModulesEx(win_pid, p_modules, MODULE_LIST_MAX_LENGTH,
                                  &n_modules, 0x03)) {
        ...
        for (int i = 0; i < n_modules; i++) {
                // TODO: How handle error?
                GetModuleFileNameExA(win_pid, p_modules[i], filename,
                                     FILENAME_MAX_LENGTH);

                if (std::string(filename).find(m_name) != std::string::npos) {
                        MODULEINFO tmp_module;
                        // TODO: How handle error?
                        GetModuleInformation(win_pid, p_modules[i], &tmp_module,
                                             sizeof(MODULEINFO));

                        m_info->address = reinterpret_cast<uint32_t>(
                                tmp_module.lpBaseOfDll);
