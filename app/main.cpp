               ...
               strcmp(user_command, "exit")) {
                ...
                if (strcmp(user_command, "in") == 0) {
                        ...
                        if (value_current == nullptr)
                                result_in =
                                        find_class(assembly_addr, user_value,
                                                   value_list[value_index]);
                        else
                                result_in = get_field_value(
                                        *value_current, user_value,
                                        value_list[value_index]);

                        if (result_in != EXE_OK) {
                                printf("ERROR: Cannot find class or field "
                                       "\"%s\".\n",
                                       user_value);
                        } else {
                ...
                if (strcmp(user_command, "list") == 0) {
                        if (value_current == nullptr) {
                                ...
                                int count = get_class_list(assembly_addr,
                                                           class_list);
                                ...
                                int count = get_field_list(*value_current,
                                                           field_list);
                                char buffer_name[DEF_BUFFER_STR];
                ...
                if (strcmp(user_command, "at") == 0 &&
                    value_current != nullptr) {
                        get_array_value(*value_current, atoi(user_value),
                                        value_list[value_index]);
                        value_current = &(value_list[value_index]);
                        value_index++;
                }
